...
function isDoubleNaN(d) {
    return [1,realWorld,isNaN(d)];
}
var isFloatNaN = isDoubleNaN;

function isDoubleInfinite(d) {
    return [1,realWorld,d === Infinity];
}
var isFloatInfinite = isDoubleInfinite;

function isDoubleNegativeZero(x) {
    return [1,realWorld,x===0 && (1/x)===-Infinity];
}
var isFloatNegativeZero = isDoubleNegativeZero;

function strEq(a, b) {
    return [1, realWorld, a == b];
}
...
function strOrd(a, b) {
    ...
    return [1, realWorld, [1, ord]];
}

function jsCatch(act, handler) {
    try {
        return [1,realWorld,A(act,[realWorld])[2]];
    } catch(e) {
        return [1,realWorld,A(handler,[e, realWorld])[2]];
    }
}

function hs_eqWord64(a, b) {
    return [1,realWorld,a[0]==b[0] && a[1] == b[1]];
}

// Word64# representation: (Word, Word)
function hs_wordToWord64(w) {
    return [1,realWorld,[0,w]];
}
...
function __word_encodeDouble(d, e) {
    return [1,realWorld, d * Math.pow(2,e)];
}
