...
function jsPrompt(str) {
    ...
    return [1,val == undefined ? '' : val.toString()];
}

function jsEval(str) {
    var x = eval(str);
    return [1,x == undefined ? '' : x.toString()];
}
...
function jsShowI(val) {return [1,val.toString()];}
function jsShow(val) {
    var ret = val.toString();
    return [1,val == Math.round(val) ? ret + '.0' : ret];
}
...
function jsGetStyle(elem, prop) {
    return [1,elem.style[prop].toString()];
}
...
function jsCreateElem(tag) {
    return [1,document.createElement(tag)];
}
...
function jsGetLastChild(elem) {
    ...
    for(var i = len-1; i >= 0; --i) {
        if(typeof elem.childNodes[i].tagName != 'undefined') {
            return [1,[2,[1,elem.childNodes[i]]]];
        }
    }
    return [1,[1]];
}
...
function u_towlower(charCode) {
    return [1, String.fromCharCode(charCode).toLowerCase().charCodeAt(0)];
}

function u_towupper(charCode) {
    return [1, String.fromCharCode(charCode).toUpperCase().charCodeAt(0)];
}
