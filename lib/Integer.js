...
Integer.fromString = function(str, opt_radix) {
  ...
  if (str.charAt(0) == '-') {
    return I_negate(Integer.fromString(str.substring(1), radix));
...
var I_decodeDouble = function(x) {
  if(isNaN(x)) {
      return [1, 972, Integer.fromString('-6755399441055744')];
  }
  var sig = x > 0 ? 1 : -1;
  if(!isFinite(x)) {
      return [1, 972, I_fromNumber(sig) * Integer.fromString('4503599627370496')];
  }
  x = Math.abs(x);
