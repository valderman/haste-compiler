...
--   selector.
withElemsQS :: (Functor m, MonadIO m) => Elem -> QuerySelector -> ([Elem] -> m a) -> m a
withElemsQS el = J.withElemsQS el . toJSStr

-- | Map an IO computation over the list of elements matching a query selector.
mapQS :: (Functor m, MonadIO m) => Elem -> QuerySelector -> (Elem -> m a) -> m [a]
mapQS el = J.mapQS el . toJSStr

-- | Like @mapQS@ but returns no value.
mapQS_ :: (Functor m, MonadIO m) => Elem -> QuerySelector -> (Elem -> m a) -> m ()
