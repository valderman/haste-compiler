...
putFloat32le f = PutM $ \a -> push a (f2ab f)
  where
    f2ab :: Float -> Unpacked
    f2ab = unsafePerformIO . ffi "(function(f) {var a=new ArrayBuffer(4);new DataView(a).setFloat32(0,f,true);return a;})"

putFloat64le :: Double -> Put
putFloat64le f = PutM $ \a -> push a (f2ab f)
  where
    f2ab :: Double -> Unpacked
    f2ab = unsafePerformIO . ffi "(function(f) {var a=new ArrayBuffer(8);new DataView(a).setFloat64(0,f,true);return a;})"
...
toAB view size elem =
    unsafePerformIO $ go view size (unpack elem)
  where
    go = ffi "window['toABle']"
